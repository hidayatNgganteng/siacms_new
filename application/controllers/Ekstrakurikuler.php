...
class Ekstrakurikuler extends CI_Controller {
	function __construct(){
		...
		if (($this->session->userdata('jabatan') != 'Kurikulum')&&($this->session->userdata('jabatan') != 'Superadmin')&&($this->session->userdata('jabatan') != 'Kesiswaan')&&($this->session->userdata('jabatan') != 'Guru')&&($this->session->userdata('jabatan') != 'Pegawai')&&($this->session->userdata('jabatan') != 'Kepsek')&&($this->session->userdata('jabatan') != 'Ekskul')) {
	...
	public function index()
	{
		...
		$this->template->load('ekstrakurikuler/dashboard','ekstrakurikuler/home' , $data);
	}
	...
	public function profile()
	{
		...
		$this->template->load('ekstrakurikuler/dashboard','pegawai/profile', $data);
	}
	...
	public function editprofil(){
		...
		$this->template->load('ekstrakurikuler/dashboard','ekstrakurikuler/editprofil', $data);
		if($this->input->post('submit')){
			...
			redirect('ekstrakurikuler/editprofil');
		} 
	}
...
public function updatepassword() {
		...
		if ($passwordbaru == $confirmpassword) {
			...
			if ($hasil > 0){
				...
				$this->session->set_flashdata('warning','<script>swal("Berhasil!", "Data Berhasil Disimpan", "success")</script>');
				redirect('ekstrakurikuler/gantipassword');
			}else{
	...
	public function distribusi_reg(){
		...
		$data['foto'] = $this->session->foto;
		$data['username'] = $this->session->username;  
		$this->template->load('ekstrakurikuler/dashboard','superadmin/kesiswaan//distribusi_reg' ,$data);
	}
...
// tutup anggrek
	...
	public function ekstrakurikuler($id_jadwal_ekskul = "")
	{
		...
		if ($id_jadwal_ekskul == "" ) {
			...
			$this->template->load('ekstrakurikuler/dashboard','ekstrakurikuler/penjadwalan/kurikulum/ekstrakurikuler', $data);
		}
	}

	public function printjadwalekskul() //$id_tahun_ajaran="")
	{
		$data['nama'] = $this->session->Nama;
		$data['foto'] = $this->session->foto; 
		...
		$this->template->load('kurikulum/dashboard','kurikulum/penjadwalan/kurikulum/printjadwalekskul', $data);
	}
	...
	public function hapusjadwalekskul() {
		...
		redirect('ekstrakurikuler/ekstrakurikuler');
	}
