...
class Nonakademik extends CI_Controller {
	...
	public function index()
	{
		...
		$this->template->load('nonakademik/dashboard','nonakademik/home', $data);
	}
	...
	function pendaftaran($action = null)
	{
		...
		if ($action == null)
		{
			...
			$this->template->load('nonakademik/dashboard','nonakademik/pendaftaran', $data);
	...
	function jadwal()
	{
		...
		$this->template->load('nonakademik/dashboard','nonakademik/jadwal', $data);
	}
	...
	function presensi($action = null)
	{
		...
		if ($action == null)
		{
			...
			$data["data_ekskul"] = $this->me->data_ekskul();
			$this->template->load('nonakademik/dashboard','nonakademik/presensi', $data);
	...
	function nilai($id_kelas_reguler = "", $idthn = "")
	{
		...
		// $data['record'] = $this->mod_siswa_kelas_reguler_berjalan->get_all_kelas();
		 ...
		 $data['foto'] = $this->session->foto; 
		 $data['username'] = $this->session->username;
		$this->template->load('nonakademik/dashboard','nonakademik/nilai', $data);
	}

	function simpan_nilai($id_kelas_reguler = "")
	{
		$this->load->model('mod_siswa_kelas_reguler_berjalan');
		$this->load->model('mod_nilaiekskul');
	...
	function pembayaran(){
		...
		$data['foto'] = $this->session->foto; 
		$data['username'] = $this->session->username;
		$this->load->model('Mod_danamandiri');
		$tgl = date('Y-m-d');
	...
	function prestasi()
	{
		...
		$this->template->load('nonakademik/dashboard','nonakademik/prestasi', $data);
	}
